...
function callHaiku(prompt, fallback, options = {}) {
  const safeBudget = Number(options.budget || 0.05);
  if (isNaN(safeBudget) || safeBudget <= 0 || safeBudget > 1) {
    return typeof fallback === 'object' ? fallback : {};
  }
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const result = execFileSync('claude', [
        ...
        '--max-turns', '1',
        '--max-budget-usd', String(safeBudget),
        '--output-format', 'json'
      ], {
        input: prompt,
        encoding: 'utf-8',
